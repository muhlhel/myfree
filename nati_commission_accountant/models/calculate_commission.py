...
class CalculateCommissionAccountant(models.TransientModel):
    _inherit = 'create.commission.wiz'

    def generate_commission(self, commission_datain):
        #raise ValidationError(_("commission Accountant"))

        accountant = self.generate_accountant_person_commission()
        commission_data = Counter(accountant) + commission_datain
        super(CalculateCommissionAccountant, self).generate_commission(commission_data)

    def generate_accountant_person_commission(self):
        ...
        config_id = self.env['accountant.person.commission'].search([], order='id desc', limit=1)

        if not config_id:
            raise ValidationError(
                _("Please Do Accountatn Person Commission Configuration under  Commission -> Configuration -> Accountant Commission."))
        pos_orders =[]
        '''
        ...
        for so_order in sale_orders:
            if so_order.accountant_per_id:
                user = so_order.accountant_per_id
            else:
                user = so_order.partner_id.accountant_per_id
        ...
        for pos_order in pos_orders:
            if pos_order.accountant_per_id:
                user = pos_order.accountant_per_id
            else:
                user = pos_order.partner_id.accountant_per_id
        ...
        for usr in user_data:
            ...
            prev_comm = self.env['commission.base'].search([
                ('commission_date', '>=', start), ('commission_type', '=', 'accountant'),
