...
class CalculateCommissionTechnical(models.TransientModel):
    _inherit = 'create.commission.wiz'

    def generate_commission(self, commission_datain):
        tech = self.generate_technical_person_commission()
        commission_data = Counter(tech) + commission_datain
        super(CalculateCommissionTechnical, self).generate_commission(commission_data)

    def generate_technical_person_commission(self):
        ...
        config_id = self.env['tech.person.commission'].search([], order='id desc', limit=1)

        if not config_id:
            raise ValidationError(
                _("Please Do Technical Person Commission Configuration under  Commission -> Configuration -> Technical Configuration."))
        pos_orders =[]
        '''
        ...
        for so_order in sale_orders:
            if so_order.technical_per_id:
                user = so_order.technical_per_id
            else:
                user = so_order.partner_id.technical_per_id
        ...
        for pos_order in pos_orders:
            if pos_order.technical_per_id:
                user = pos_order.technical_per_id
            else:
                user = pos_order.partner_id.technical_per_id
        ...
        for usr in user_data:
            ...
            prev_comm = self.env['commission.base'].search([
                ('commission_date', '>=', start), ('commission_type', '=', 'tech_person'),
